...
class ReactionChemformer(nn.Module):
    ...
    def forward(self, reactants: List[List[str]], products: List[str]) -> List[float]:
        ...
        _reactants = [Bag(Molecule(s) for s in r) for r in reactants]
        _products = [Molecule(s) for s in products]
        output = self.model(_reactants, num_results=self.num_results)

        probs = []
        assert len(output) == len(_products)
        for reaction_list, product in zip(output, _products):
            prob = 0.0
            for reaction in reaction_list:
                predicted_product_bag = reaction.products
                if product in predicted_product_bag:
                    prob += reaction.get_prob()
            probs.append(prob)
