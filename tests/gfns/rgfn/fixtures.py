...
def fragment_path() -> Path:
    return Path(__file__).parent / "../../../data/fragments_nodocking.csv"


@pytest.fixture(scope="module")
def fragment_path_docking() -> Path:
    return Path(__file__).parent / "../../../data/fragments_docking.csv"
...
def rgfn_data_factory(reaction_path: Path, fragment_path: Path) -> ReactionDataFactory:
    return ReactionDataFactory(
        reaction_path=reaction_path, fragment_path=fragment_path, docking=False
    )
...
def rgfn_data_factory_docking(
    ...
    return ReactionDataFactory(
        reaction_path=reaction_path, fragment_path=fragment_path_docking, docking=True
    )
